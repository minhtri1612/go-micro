...
bootstrap GitOps từng manifest (chi tiết tùy chọn: kind/README.md cho lab Kind), External Secrets Operator (dev/prod).
...
# /etc/hosts entries → ALB (Ingress) cho từng env; khớp hostnames trong config/env/*.yaml (HTTPRoute).
HOSTNAMES_FOR_ALB_BY_ENV = {
    "management": ("argocd.local",),
    "dev": ("dev-go-micro.local",),
    "prod": ("prod-go-micro.local",),
}

# Thứ tự bootstrap trên cluster management — kubectl apply từng file (Argo app từng manifest; repo dùng Envoy Gateway trên RKE2/EC2).
_ARGOCD_BOOTSTRAP_FILES = [
    "00-argocd-cm-health.yaml",
    "01-projects.yaml",
    "05-monitoring-mgmt.yaml",
    "06-monitoring-dev.yaml",
    "08-monitoring-prod.yaml",
    "09-cilium-dev.yaml",
    "11-cilium-prod.yaml",
    "18-cilium-management.yaml",
    "12-argo-rollouts-dev.yaml",
    "14-argo-rollouts-prod.yaml",
    "19-envoy-gateway-dev.yaml",
    "23-envoyproxy-nodeport-dev.yaml",
    "21-envoy-gateway-prod.yaml",
    "24-envoyproxy-nodeport-prod.yaml",
    "25-gateway-infra-dev.yaml",
    "27-gateway-infra-prod.yaml",
    "02-dev-microservices-stack.yaml",
    "04-prod-microservices-stack.yaml",
    "22-jenkins-mgmt.yaml",
]
...
def apply_external_secrets_manifests():
    ...
    # Namespaces khớp template Helm + config/env (microservices-<env>, databases-<env>).
    for ns in (f"microservices-{TERRAFORM_ENV}", f"databases-{TERRAFORM_ENV}", "external-secrets"):
        subprocess.run(
    ...
    if not os.path.isdir(chart_dir) or not os.path.isfile(values_chart):
        print(f"  ⚠ Helm chart external-secrets/applications not found. Skipping apply.")
        print(f"    Expected: {values_chart}")
        return
    if not os.path.isfile(values_config_base) or not os.path.isfile(values_config_env):
        print(f"  ⚠ Missing config merge files. Skipping apply.")
        print(f"    Expected: {values_config_base} and {values_config_env}")
        return
...
def deploy_argocd_bootstrap_manifests():
    """kubectl apply từng file argocd/bootstrap/* — cùng tinh thần kind/README (không dùng root ApplicationSet)."""
    print("--- Step 3: Deploying Argo CD bootstrap manifests (per-file) ---")
    kubeconfig_path = _kubeconfig_for_deploy()
    env = os.environ.copy()
    env["KUBECONFIG"] = kubeconfig_path
    ...
    applied = 0
    for name in _ARGOCD_BOOTSTRAP_FILES:
        path = os.path.join(bootstrap_dir, name)
        if not os.path.isfile(path):
            print(f"  ⏭ Skip (missing): {name}")
            continue
        print(f"  Applying {name} ...")
        run_command(f"kubectl apply -f {path}", env=env, timeout=120)
        applied += 1
    if applied == 0:
        print(f"  ✗ No bootstrap YAMLs applied under {bootstrap_dir}")
        sys.exit(1)
    print(f"  ✓ Applied {applied} bootstrap manifest(s).")
    print("  📌 Tiếp theo: thêm Helm repo + argocd app sync / app wait (xem kind/README.md mục 4).")
...
def main():
    ...
    if TERRAFORM_ENV == "management":
        ...
        deploy_argocd_bootstrap_manifests()
        print("\n  ─── Argo CD cluster registration (spoke clusters) ───")
        print("  Sau provision dev/prod + configure.py dev|prod, chạy (nếu cần):")
        ...
        if TERRAFORM_ENV == "prod":
            ...
            if os.path.isfile(mgmt_kube):
                trigger_argocd_app_sync(["dev-microservices", "prod-microservices"], mgmt_kube)
    ...
    if TERRAFORM_ENV == "management":
        ...
        host = f"{TERRAFORM_ENV}-go-micro.local"
        print(f"\n  🌐 HTTPRoute / Gateway (ví dụ):  https://{host}")
